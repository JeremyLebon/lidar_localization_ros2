...
void PCLLocalization::cloudReceived(const sensor_msgs::msg::PointCloud2::SharedPtr msg)
{
 ...
 if (!msg) {
        RCLCPP_ERROR(this->get_logger(), "Received null PointCloud2 message. Skipping processing.");
        return;
    }

    if (msg->data.empty()) {
        RCLCPP_ERROR(this->get_logger(), "Received empty PointCloud2 message. Skipping processing.");
        return;
    }
    RCLCPP_INFO(this->get_logger(), "Received valid PointCloud2 message.");
  ...
  RCLCPP_INFO(this->get_logger(), "Received PointCloud2 Message:");
    RCLCPP_INFO(this->get_logger(), "Header frame: %s", msg->header.frame_id.c_str());
    RCLCPP_INFO(this->get_logger(), "Timestamp (sec): %u, (nanosec): %u", msg->header.stamp.sec, msg->header.stamp.nanosec);
    RCLCPP_INFO(this->get_logger(), "Height: %u, Width: %u", msg->height, msg->width);
    RCLCPP_INFO(this->get_logger(), "Point Step: %u, Row Step: %u", msg->point_step, msg->row_step);
    RCLCPP_INFO(this->get_logger(), "Number of fields: %lu", msg->fields.size());
  ...
  pcl::PointCloud<pcl::PointXYZ>::Ptr cloud__xyz_ptr(new pcl::PointCloud<pcl::PointXYZ>);
  pcl::PointCloud<pcl::PointXYZI>::Ptr cloud_ptr(new pcl::PointCloud<pcl::PointXYZI>);
  RCLCPP_INFO(get_logger(), "pointer declared");
  ...
  try {
     pcl::fromROSMsg(cloud_msg, *cloud__xyz_ptr);
     
    } catch (const std::exception& e) {
        RCLCPP_ERROR(rclcpp::get_logger("PointCloudConversion"), "Exception: %s", e.what());
        
    }
  RCLCPP_INFO(get_logger(), "msg converted to pcl cloud");
  // Iterate through the input cloud and add intensity
        for (const auto& point : cloud__xyz_ptr->points) {
            pcl::PointXYZI point_xyzi;
            point_xyzi.x = point.x;
            point_xyzi.y = point.y;
            point_xyzi.z = point.z;
            point_xyzi.intensity = 0.0f; // Set default intensity value, can be modified as needed

            cloud_ptr->points.push_back(point_xyzi);
        }



  
  RCLCPP_INFO(get_logger(), "Message is read");
